...
//Rare exceptions to indicate a bug in the _programmingassignment4 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_programmingassignment4 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_programmingassignment4]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.programmingassignment4.module"
