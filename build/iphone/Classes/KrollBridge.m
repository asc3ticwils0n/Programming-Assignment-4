...
NSString * ProgrammingAssignment4$ModuleRequireFormat = @"(function(exports){"
...
@implementation ProgrammingAssignment4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_programmingassignment4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ProgrammingAssignment4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ProgrammingAssignment4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_programmingassignment4 = [[ProgrammingAssignment4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_programmingassignment4];
	
	NSString *_programmingassignment4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _programmingassignment4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_programmingassignment4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ProgrammingAssignment4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.programmingassignment4.kroll" 
